...
#: ``FACTORYFLOW_LLM_PROFILE``) so nobody points at a 60 GB model by accident.
...
# ---------------------------------------------------------------------------
# Model profiles — master doc 6b.3 / Part 1 memory budget
# ---------------------------------------------------------------------------
...
#: was launched. Confirm the two Nemotron ids against
#: ``factoryflow_gb10_model_downloads.md`` on the day; both are overridable with
#: ``FACTORYFLOW_LLM_MODEL`` without touching this table.
...
#: here so a hopeful ``FACTORYFLOW_LLM_MODEL`` fails in milliseconds rather than
...
def _reject_ultra(model: str) -> None:
    if _ULTRA.search(model):
        raise LLMError(
            ...
            "at NVFP4, ~2.5x the GB10's 128 GB of unified memory (master doc Part 1). "
...
def resolve_profile(name: str | None = None) -> ModelProfile:
    ...
    Precedence: explicit argument, ``$FACTORYFLOW_LLM_PROFILE``, then the laptop
    default. ``FACTORYFLOW_LLM_URL`` / ``_MODEL`` / ``_TIMEOUT`` override the
    chosen profile's fields, so the same command line works on both machines.
    """
    key = name or os.environ.get("FACTORYFLOW_LLM_PROFILE") or DEFAULT_PROFILE
    ...
    model = os.environ.get("FACTORYFLOW_LLM_MODEL", profile.model)
    _reject_ultra(model)

    timeout_raw = os.environ.get("FACTORYFLOW_LLM_TIMEOUT")
    try:
        timeout_s = float(timeout_raw) if timeout_raw else profile.timeout_s
    except ValueError as exc:
        raise LLMError(f"FACTORYFLOW_LLM_TIMEOUT is not a number: {timeout_raw!r}") from exc

    return ModelProfile(
        name=profile.name,
        model=model,
        base_url=os.environ.get("FACTORYFLOW_LLM_URL", profile.base_url),
...
class OpenAICompatBackend:
    ...
    def from_profile(
        ...
        return cls(
            ...
            model=profile.model,
            api_key=api_key if api_key is not None else os.environ.get("FACTORYFLOW_LLM_KEY"),
    ...
    def _post(self, path: str, payload: dict[str, object], timeout_s: float) -> dict[str, object]:
        ...
        raise LLMError(
            ...
            "preflight it before the demo beat (master doc Part 1)."
        )
...
class FallbackBackend:
    ...
    The README has always claimed :class:`FakeBackend` is the fallback if the
    GB10's model server misbehaves. Part 1's swap rule makes that likely rather
    than hypothetical — the model is deliberately unloaded for training and
