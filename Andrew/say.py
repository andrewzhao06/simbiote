...
`Andrew/hospital_server.py` has to already be up in another shell. This is a
...
the system interpreter and returns as soon as the robot finishes.
    ...
    python3 Andrew/say.py --status
    python3 Andrew/say.py "go to room two" "then the supply room"
"""
...
if args.status or not args.instructions:
    if status is None:
        print("sim is not running (no status file). Start Andrew/hospital_server.py")
        raise SystemExit(1)
