...
_ENV_VARS = (
    "SIMBIOTE_LLM_PROFILE",
    "SIMBIOTE_LLM_URL",
    "SIMBIOTE_LLM_MODEL",
    "SIMBIOTE_LLM_TIMEOUT",
    "SIMBIOTE_LLM_KEY",
)
...
def clean_env(monkeypatch):
    """A developer's own SIMBIOTE_LLM_* settings must not steer these tests."""
...
def _backend(monkeypatch, urlopen: _Urlopen, **kwargs: object) -> OpenAICompatBackend:
    monkeypatch.setattr("simbiote.agentic.llm_backend.urllib.request.urlopen", urlopen)
...
def test_env_selects_the_profile(monkeypatch):
    monkeypatch.setenv("SIMBIOTE_LLM_PROFILE", "nemotron-nano")
...
def test_env_overrides_url_model_and_timeout(monkeypatch):
    ...
    monkeypatch.setenv("SIMBIOTE_LLM_URL", "http://gb10:8000/v1")
    monkeypatch.setenv("SIMBIOTE_LLM_MODEL", "nemotron-3-super")
    monkeypatch.setenv("SIMBIOTE_LLM_TIMEOUT", "42")
...
def test_nonnumeric_timeout_env_is_a_clear_error(monkeypatch):
    monkeypatch.setenv("SIMBIOTE_LLM_TIMEOUT", "soon")
...
def test_ultra_is_refused_wherever_it_is_named(monkeypatch, model):
    ...
    monkeypatch.setenv("SIMBIOTE_LLM_MODEL", model)
...
def test_no_fallback_leaves_the_failure_hard(monkeypatch, scene):
    ...
    monkeypatch.setattr(
        "simbiote.agentic.llm_backend.urllib.request.urlopen", _Urlopen(_refused())
    )
