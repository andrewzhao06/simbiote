...
## Part 6 — Step 3: Hand-Tracking Teleoperation
**(Teammate 3's step)**

### 6.1 What this step does

A human operator's tracked hand movements remotely drive the robot's base and arm in real time, producing a `RobotAction` each frame. Every session is logged as a demonstration that feeds Step 2's fine-tune loop.
...
└── actions.py   — RobotAction(base_velocity: (vx, vy, omega),
                    ...
                    You and Teammate 4 are both producers of this schema —
                    agree on it together before either of you builds
                    against it, don't design it in isolation.
```

### 6.3 Camera input — confirmed: Iriun Webcam, using the same iPhone as Step 1

Iriun turns the iPhone into a standard virtual webcam — every hand-tracking model below reads from it via a plain `cv2.VideoCapture` call, nothing custom to build for acquisition itself.

| | Laptop (today) | GB10 (tomorrow) |
| :---- | :---- | :---- |
| Connection | USB (preferred — ~1–5 ms latency, doesn't depend on Wi-Fi) or Wi-Fi | Same — Iriun has Linux support, covers DGX OS |
| Resolution/frame rate | 720p @ 30 fps — this stage is latency-bound, not quality-bound | Same setting; no reason to raise it even with GB10's compute |

Setup task for tonight: install the Iriun app on the iPhone and the Iriun desktop client on the laptop (admin rights needed), connect via USB, confirm "Iriun Webcam" appears as a selectable camera device.

### 6.4 Models — laptop (today) vs GB10 (tomorrow)

| Component | Laptop (today) | GB10 (tomorrow) |
| :---- | :---- | :---- |
| Hand-pose estimation | **MediaPipe Hands** — real-time 2D+rough-3D landmarks, CPU, no GPU needed | **WiLoR** — transformer/ViT-based 3D hand mesh recovery (MANO model), end-to-end detection, much higher finger-articulation fidelity |
| IK / motion retargeting | Simple analytic/Jacobian IK solver (CPU) | **cuMotion / cuRobo** (CUDA), collision-aware real-time re-planning |
| Imitation-learning fine-tune | A few short demo trajectories against the Step 2 laptop-scale policy, one small behavior-cloning pass | Same logic against the full-scale Step 2 policies, much larger/faster demonstration ingestion |

**Practical note:** MediaPipe Hands is a legitimate production fallback, not just a placeholder — keep both wired behind the same `hand_tracking.py` interface and decide which demos better once you can measure real GB10 latency.
...
│                        (MediaPipe today → WiLoR tomorrow, same signature)
├── ik_bridge.py        — def landmarks_to_action(landmarks) -> RobotAction
│                        (analytic IK today → cuMotion/cuRobo tomorrow)
...
**Build order:** `actions.py` (with Teammate 4) → `camera_source.py` (confirm a clean Iriun feed before anything else) → `hand_tracking.py` → `ik_bridge.py` → `teleop_session.py` → `demo_logger.py` (with Teammate 4).

**Acceptance test:** move a hand, watch the simulated robot's base and arm move correspondingly in real time; `demo_logger.py` produces a trajectory file that Step 2's fine-tune step can ingest.

### 6.6 Tonight's realistic scope

Agree on `actions.py` and `demo_logger.py`'s schema with Teammate 4 first — that's a five-minute conversation that saves both of you a rewrite later. Then get Iriun connected and `camera_source.py` pulling a clean frame, then the rest of the teleop chain against that real feed and a toy stand-in robot (from Step 2's PyBullet prototype). None of this needs the GB10.

### 6.7 Handoff

**You receive from Step 2:** exported nav/grasp checkpoints aren't actually needed for teleop itself (you're driving directly), but coordinate with Teammate 2 on what stand-in robot you're driving tonight. **You produce for Step 2:** logged trajectories from `demo_logger.py` — settle this schema as a group conversation with Teammates 2 and 4, since it's the one interface all three of you touch.

---

## Part 6b — Step 4: Robot Prompting (Agentic Control)
**(Teammate 4's step)**

### 6b.1 What this step does

Instead of a human driving every movement, a typed or spoken natural-language instruction gets decomposed into a sequence of high-level skills that execute autonomously using Step 2's trained policies. Every executed session is logged as a demonstration, exactly like Step 3's teleop sessions.
...
└── actions.py   — RobotAction(base_velocity: (vx, vy, omega),
                                arm_target_pose: Pose,
                                gripper_state: open|closed)
                    You and Teammate 3 are both producers of this schema —
                    agree on it together before either of you builds
                    against it, don't design it in isolation.
```

### 6b.3 Models — laptop (today) vs GB10 (tomorrow)

| Component | Laptop (today) | GB10 (tomorrow) |
| :---- | :---- | :---- |
| Instruction parsing | Qwen3 8B or Phi-4-mini | **Nemotron 3 Super** (NVFP4, ~60 GB; needs sequencing around Isaac Sim, see Part 1) — or **Nemotron 3 Nano** (~25 GB) if you want it always resident — shared with Steps 1 and 2 |
| Skill execution | Calls Step 2's laptop-scale PyBullet-trained checkpoints (or a toy stand-in if those aren't ready yet) | Calls Step 2's real Isaac Lab-trained checkpoints |

### 6b.4 High-level task hierarchy — the piece that makes this more than a single tool call

An LLM can decompose "pick up the tray" into one tool call, but it can't safely turn "move the wheelchair to Room 2" into raw motor commands directly — that needs an explicit task hierarchy between the LLM and the low-level policies, so each step has a clear precondition and a clear success/failure signal rather than one giant opaque action.

```
"move the wheelchair to Room 2"
       │
       ▼  command_parser.py (LLM call)
[ApproachWheelchair, AlignGripper, AttachHandleConstraint, NavWithPayload("Room 2"), Detach]
       │
       ▼  task_executor.py (Behavior Tree / FSM — plain Python, no LLM per step)
   each atomic skill runs to completion or reports failure before the next one starts
```

- **Simple instructions** ("pick up the tray in the supply room") decompose to a short tool-call list — `navigate_to()` → `pick_up()` — and don't need the full tree, just `command_parser.py` → `robot_tools.py` directly.
- **Compound/stateful instructions** (anything involving the wheelchair stretch task from Step 2, §5.5) need the explicit state machine: `ApproachWheelchair` → `AlignGripper` → `AttachHandleConstraint` → `NavWithPayload` → `Detach`, where each state is a call into Step 2's `grasp_attach.py`/trained policies, and the FSM — not the LLM — decides when a state is complete and it's safe to advance.
- **Why this matters even without the wheelchair task:** if the wheelchair stretch goal doesn't get built in time, this same task-hierarchy pattern is still the right structure for any multi-step instruction, so it's worth building regardless of how far Step 2's stretch goals get.
...
│                          attach_handle() / nav_with_payload(location_id) / detach()
│                          — each runs inference on Step 2's exported checkpoints
├── command_parser.py     — parse_instruction(text) -> list[ToolCall], LLM call;
│                          for compound instructions, emits a task-hierarchy
│                          sequence rather than a single flat tool call
├── task_executor.py       — the Behavior Tree/FSM from §6b.4: runs each atomic
...
**Build order:** `actions.py` (with Teammate 3) → `scene_query.py` → `robot_tools.py` (standalone, no LLM yet — validate `navigate_to()`/`pick_up()` work by calling a Step 2 checkpoint directly) → `command_parser.py` (validate against an explicit tool-call schema, including the compound-instruction case) → `task_executor.py` → `agentic_session.py` → `demo_logger.py` (with Teammate 3).

**Acceptance tests:**
- *Simple:* type "pick up the tray in the supply room," watch the robot navigate there and grasp it unattended.
- *Compound (if the wheelchair task exists):* type "move the wheelchair to Room 2," watch `task_executor.py` step through all five states in order, each waiting for the previous to actually complete.
- *Both:* `demo_logger.py` produces a trajectory file, and Step 2's fine-tune step ingests it without modification.

### 6b.6 Tonight's realistic scope

Agree on `actions.py` and `demo_logger.py`'s schema with Teammate 3 first. Then `scene_query.py` and `command_parser.py`'s schema design (including the compound-instruction shape) can be built and tested tonight against a hand-written fake scene graph matching Step 1's real schema — you don't need real trained policies to validate that the LLM parses instructions into the right shape. `task_executor.py`'s state-transition logic can also be fully tested tonight against stub skill functions that just print and return success. `robot_tools.py`'s actual policy-inference calls are the one piece that waits on Step 2's real checkpoints tomorrow.

### 6b.7 Handoff

**You receive from Step 1:** the scene graph (via `scene_query.py`) — coordinate the JSON schema with Teammate 1 before building against it. **You receive from Step 2:** exported nav/grasp checkpoints, and if attempted, the wheelchair-task tool signatures — coordinate the inference call signatures with Teammate 2. **You produce for Step 2:** logged trajectories from `demo_logger.py`, in whatever schema Step 2's `ingest_demo()` expects — settle this schema as a group conversation, not bilaterally.
