...
$FF_PY -m pytest Gagan/tests
```

## Running it on the GB10 (verified end to end)

There is no venv for this. `Gagan/env.gb10.sh` points at Isaac Sim's bundled
...
$FF_PY -m factoryflow_mapper.cli --config config/mapper.gb10.toml \
    ingest UPLOAD_PHONE_SCANS_HERE/<scan>

# 2. scan -> USD
$FF_PY -m factoryflow_mapper.cli --config config/mapper.gb10.toml run \
    --capture UPLOAD_PHONE_SCANS_HERE/<scan> \
    --out Gagan/out/<scan>.usda

# 3. SCAN_MAP.md 4.5 acceptance test: does it open in Isaac Sim?
/home/dell/IsaacSim/_build/linux-aarch64/release/python.sh \
    Gagan/check_isaac_sim.py Gagan/out/<scan>.usda      # add --gui to watch it
```
...
`check_isaac_sim.py` asserts this and then drops a rigid body to prove the
...
### Real graspable objects — `Gagan/adapters/sam3_detect.py`
...
source Gagan/env.gb10.sh
export FACTORYFLOW_MODE=preview
export FACTORYFLOW_SAM3_COMMAND="$FF_PY $PWD/Gagan/adapters/sam3_detect.py \
    --capture {capture} --geometry {geometry} \
    --checkpoint {checkpoint} --output {output} \
    --config $PWD/config/mapper.gb10.toml --frames 16"

$FF_PY -m factoryflow_mapper.cli --config config/mapper.gb10.toml run \
    --capture UPLOAD_PHONE_SCANS_HERE/<scan> --out Gagan/out/<scan>.usda
```
