...
def test_trajectories_to_dataset_nav_shapes():
    trajs = [make_toy_trajectory(f"s{i}", obs_dim=7, length=10, task="nav", seed=i) for i in range(3)]
...
def test_task_action_vector_grasp_delta():
    prev = None
    action = RobotAction(arm_target_pose=Pose(position=(0.1, 0.0, 0.2)), gripper_state=GripperState.OPEN)
...
def test_train_bc_produces_loadable_checkpoint(tmp_path):
    trajs = [make_toy_trajectory(f"s{i}", obs_dim=5, length=20, task="nav", seed=i) for i in range(4)]
...
def test_train_bc_warm_starts_existing_policy(tmp_path):
    ...
    assert not torch.allclose(before, after), "train_bc should update the passed-in policy_net in place"
