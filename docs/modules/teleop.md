...
The implementation lives in `simbiote/teleop/` and `simbiote/sim_stub/`. Tests live in
`tests/teleop/` and run as part of the root `pytest` suite.
...
- `scripts/teleop/run_demo.py` — convenience entry point, equivalent to
...
`tests/teleop/test_teleop_logic.py` — smoke tests for the retargeting logic and
