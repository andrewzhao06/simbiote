...
README "Known issues"). `tests/test_sim_env.py` covers the PyBullet-specific
...
from simbiote.training.ppo import PPOConfig, VecEnvList, compute_gae, train_ppo
...
def test_vec_env_list_reset_and_step_shapes():
    ...
    obs2, rewards, terminated, truncated, infos = vec.step(actions)
...
def test_train_ppo_smoke_runs_without_crashing():
    ...
    config = PPOConfig(total_timesteps=64, rollout_steps=32, train_iters=1, minibatch_size=16, seed=0)

    updates_seen = []
    trained = train_ppo([_make_pendulum, _make_pendulum], policy, config, progress_callback=updates_seen.append)
...
def test_train_ppo_improves_mean_return_on_pendulum():
    ...
    policy = ActorCriticMLP(meta)
    config = PPOConfig(total_timesteps=1536, rollout_steps=256, train_iters=6, minibatch_size=64, lr=3e-3, seed=0)
    ...
    assert last_return > first_return, f"expected improvement, got first={first_return} last={last_return}"
