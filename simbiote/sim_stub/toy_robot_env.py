...
(spec §5.4/§6.7). This module exists only so teleop_session.py has
something concrete to drive end to end before that lands; swap it out for
Step 2's real sim binding (`simbiote.sim_env`) as soon as it's available.
...
class ToyRobotEnv:
    ...
    def __init__(self, gui: bool = True, spawn_position=(0.0, 0.0, 0.3)):
        ...
        # Last commanded arm pose, held when a frame carries no arm command
        # (e.g. `arm_target_pose=None` for a pure-nav action).
        self._last_arm_pose = Pose(position=(0.3, 0.0, 0.4))
